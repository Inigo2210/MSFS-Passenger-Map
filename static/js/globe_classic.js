...
function updateAircraft(lat, lon, alt, headingDeg) {
  ...
  const hpr = new Cesium.HeadingPitchRoll(
    Cesium.Math.toRadians((_aircraftHeading + HEADING_MODEL_OFFSET_DEG) % 360),
    0, 0
  );
...
function pushTrailPosition(lat, lon, alt) {
  ...
  _trailPositionsArray.push(Cesium.Cartesian3.fromDegrees(lon, lat, 0));
  ensureTrailEntity();
}
...
function drawDashedToDestination(from, to) {
  try {
    ...
    if (routeEntity && routeEntity.polyline && !isSimBriefRoute) {
      _routePositionsArray.length = 0;
      surfacePositions.forEach(p => _routePositionsArray.push(p));
      return;
    }

    if (routeEntity) viewer.entities.remove(routeEntity);
    _routePositionsArray = [...surfacePositions];
...
function drawSimBriefPoly(points) {
  ...
  if (routeEntity && isSimBriefRoute && _lastSimBriefKey === simKey) return;
      ...
      if (currentRoute?.destination) {
        if (currentRoute.simbriefWaypoints) {
          ...
          drawDashedToDestination(
            { lat: lastKnown.lat, lon: lastKnown.lon, alt: 0 },
...
async function loadSimBrief(silent = false) {
  ...
  try {
    ...
    currentRoute = {
      ...
      destination: { city: destData.city,   name: destData.name,   iata: destData.iata,   icao: destData.icao,   lat: destData.lat,   lon: destData.lon,   tz: destData.tz   },
      labelType,
...
function updateTerrainBtn() {
  ...
  btn.textContent           = t.flightPath3DOffLabel   || '3D Flight Path (beta): OFF';
  btn.style.backgroundColor = '#555';
  btn.title                 = t.flightPath3DOffTooltip || '3D Flight Path mode is OFF. Click to enable — beta feature, may have bugs and impact performance.';
}

function toggleTerrainBtn() {
  const btn = document.getElementById("terrainToggleBtn");
  if (btn && btn.disabled) return;
  // Switching to 3D mode requires a personal Cesium Ion token
...
function setModeSwitchDisabled(disabled) {
  ...
  btn.title             = disabled
    ? (t.flightPath3DDisabledTooltip || "Cannot change mode while a route is active — reset first.")
    : (t.flightPath3DOffTooltip      || '3D Flight Path mode is OFF. Click to enable — beta feature, may have bugs and impact performance.');
}
...
function initSettingsPanel() {
  ...
  const terrainCb = document.getElementById("terrain3DToggle");
  updateTerrainBtn();
  ...
  if (iconSel) {
    const savedModel = localStorage.getItem("globeAircraftModel") || "A320";
